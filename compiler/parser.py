...
def parse_expr(tokens, (ptr, file)):
    ...
    if tokens[ptr].obj in prefix_map:
        ...
        return common.Form([common.Symbol(s=("", prefix_map[tokens[ptr].obj])), res]), ptr2
        ...
        raise SyntaxError("Unexpected `%s`" % tokens[ptr].obj, (file, tokens[ptr].meta.row, tokens.meta.col, tokens[ptr].obj))

@loud
def parse_array(tokens, (ptr, file)):
    if tokens[ptr].obj == "\n":
        res, ptr2 = parse_layout(tokens, (ptr+1, file))
        return common.Array(res), ptr2
    else:
        ptr0 = ptr
        values = []
        while tokens[ptr].obj != "]":
            ...
            values.append(res)
        return common.Array(values), ptr
...
def parse_hash(tokens, (ptr, file)):
    if tokens[ptr].obj == "\n":
        ...
        return common.Hash(dict((elem[0], elem[1]) for elem in res)), ptr2
    else:
        ptr0 = ptr
        values = {}
        while tokens[ptr].obj != "}":
